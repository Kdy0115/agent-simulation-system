...
class CanvasGrid_3d(CanvasGrid):
    ...
    def __init__(
        ...
        """
        super().__init__(portrayal_method, grid_width, grid_height, canvas_width=500, canvas_height=500,)

    def render(self, model):
        grid_state = defaultdict(list)
        for x in range(model.grid.width):
            for y in range(model.grid.height):
                cell_objects = model.grid.get_cell_list_contents([(x, y, 1)])
